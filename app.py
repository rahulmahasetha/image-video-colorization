...
# ---------- Metrics computation ----------
def compute_frame_metrics(original_rgb, predicted_rgb):
    """Compute PSNR, SSIM, mean Delta E, RGB accuracy for two RGB images (uint8)."""
    orig = original_rgb.astype(np.float32)
    pred = predicted_rgb.astype(np.float32)

    psnr_val = psnr(orig, pred, data_range=255)
    ssim_val = ssim(orig, pred, multichannel=True, data_range=255, channel_axis=2)

    orig_lab = rgb2lab(orig / 255.0)
    pred_lab = rgb2lab(pred / 255.0)
    delta_e_map = deltaE_ciede2000(orig_lab, pred_lab)
    delta_e_mean = float(np.mean(delta_e_map))

    mae = np.mean(np.abs(orig - pred)) / 255.0
    rgb_acc = max(0.0, 1.0 - mae)
...
def process_video(input_path, gray_output_path, pred_output_path, model, device,
                  enhance_mode, manual_params=None, mode="Gray -> Predict",
    ...
    metrics_sum = {"psnr": 0.0, "ssim": 0.0, "delta_e": 0.0, "rgb_acc": 0.0}
    metrics_count = 0
    ...
    while True:
        ...
        # Original colour frame (for metrics)
        if compute_metrics and mode == "Color -> Gray -> Predict":
        ...
        # Resize predicted to original frame size
        rgb_resized = cv2.resize(rgb, (width, height), interpolation=cv2.INTER_CUBIC)

        # Compute metrics if requested
        if compute_metrics and mode == "Color -> Gray -> Predict":
            # Ensure same dimensions
            if original_rgb.shape[:2] != rgb_resized.shape[:2]:
                original_rgb = cv2.resize(original_rgb, (rgb_resized.shape[1], rgb_resized.shape[0]))
            metrics = compute_frame_metrics(original_rgb, rgb_resized)
            for k in metrics_sum:
                metrics_sum[k] += metrics[k]
            metrics_count += 1

        pred_bgr = cv2.cvtColor(rgb_resized, cv2.COLOR_RGB2BGR)
    ...
    avg_metrics = None
    if metrics_count > 0:
        avg_metrics = {k: v / metrics_count for k, v in metrics_sum.items()}

    return gray_codec, pred_codec, avg_metrics
...
if main_option == "Image":
    ...
    if uploaded:
        ...
        if image_mode == "Color -> Gray -> Predict":
            ...
            with st.expander("📊 Image Quality Metrics (Original vs Predicted)", expanded=True):
                orig_np = np.array(original)
                pred_np = np.array(result)
                metrics = compute_frame_metrics(orig_np, pred_np)
                ...
                with metric_cols[2]:
                    st.metric("Delta E (mean)", f"{metrics['delta_e']:.2f}")
                with metric_cols[3]:
                    st.metric("RGB Accuracy", f"{metrics['rgb_acc']:.4f}")
        else:
    ...
    if uploaded_video:
        ...
        if st.button("Start Colorization"):
            with st.spinner("Processing video... please wait"):
                # Enable metrics only for Color -> Gray -> Predict mode
                compute_metrics = (video_mode == "Color -> Gray -> Predict")
                gray_codec, pred_codec, avg_metrics = process_video(
                    input_path,
                    gray_output_path,
                    pred_output_path,
                    model,
            ...
            if video_mode == "Color -> Gray -> Predict":
                ...
                if avg_metrics is not None:
                    with st.expander("📊 Video Quality Metrics (average over all frames)", expanded=True):
                        metric_cols = st.columns(4)
                        with metric_cols[0]:
                            st.metric("PSNR (dB)", f"{avg_metrics['psnr']:.2f}")
                        with metric_cols[1]:
                            st.metric("SSIM", f"{avg_metrics['ssim']:.4f}")
                        with metric_cols[2]:
                            st.metric("Delta E (mean)", f"{avg_metrics['delta_e']:.2f}")
                        with metric_cols[3]:
                            st.metric("RGB Accuracy", f"{avg_metrics['rgb_acc']:.4f}")
                else:
                    st.info("Metrics could not be computed for this video.")
